...
extern "C" double* cuda_initialize(double **u_res, int nx, double *device_u_res){
    ...
    // Copy from host to device first 2 values
...
extern "C" void async_cuda_update_first_and_last(double **u_res, int nx, int pos, double *device_u_res){
    cudaMemcpy(&device_u_res[pos*(nx+2)], &u_res[pos][0], \
     2*sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(&device_u_res[pos*(nx+2)+nx], &u_res[pos][nx], \
     2*sizeof(double), cudaMemcpyHostToDevice);
}

// Get one before last value
extern "C" void async_cuda_get_first_last(double **u_res, int nx, double *device_u_res){
    cudaMemcpy(&u_res[1][2], &device_u_res[1*(nx+2)+2] , \
        sizeof(double), cudaMemcpyDeviceToHost);
    
    cudaMemcpy(&u_res[1][nx-1], &device_u_res[1*(nx+2)+nx-1] , \
        sizeof(double), cudaMemcpyDeviceToHost);
}
