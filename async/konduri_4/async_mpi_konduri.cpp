...
void at_compute_values_next_time_step(double **u_res, const int nx, const double cfl, int k, double **recv_buf){
    ...
    if(k==0){
        ...
        c[4] = -225;
        c[5] = 45;
        uxx = 71*u_res[1][2] -236*u_res[1][3] + 294*u_res[1][4] - \
        164*u_res[1][5] + 35 * u_res[1][6];

        for(int i=0; i<6;i++){
            uxx += c[i]*recv_buf[0][5-i];
        }
        uxx /= 12;
        ...
        uxx = 71*u_res[1][nx-1] -236*u_res[1][nx-2] + 294*u_res[1][nx-3] - \
        164*u_res[1][nx-4] + 35 * u_res[1][nx-5];

        for(int i=0; i<6;i++){
            uxx += c[i]*recv_buf[1][i];
        }
        uxx /= 12;
...
double **recv_buf, MPI_Comm comm){
    ...
    u_res[pos][0]=recv_buf[0][numbers-1];
...
int main(int argc, char *argv[]){
    ...
    for(int j=2; j<=nt; j++){
        ...
        if(k%DELAY==0){
            ...
            async_send_and_receive((double **)u_res, nx, 1, 6, (double **)recv_buf, comm_cart);
        }
        ...
        // if(j==2 & rank==1){
        //     cout<<"\n\n Printing recv_buf for rank "<<rank<<endl;
        //     for(int i=0; i<6; i++){
